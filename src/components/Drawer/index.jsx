...
function Drawer({items =[], onRemove, onClose, opened}) {
   ...
   const {cartItems, setCartItems, totalPrice} = useCart(); // кастомный хук юзкарт
    ...
    const onClickOrder = async () => {
      try {
        setIsLoading(true);
        const {data} = await axios.post('https://641c231ab556e431a8666273.mockapi.io/order', {items: cartItems});
      ...
      setCartItems([]);
        for ( let i = 0; i < cartItems.length; i++) {
          ...
          await axios.delete('https://6411eb8b6e3ca3175301c8a9.mockapi.io/Cart/' + item.id);
          await delay(1000);
        }
  ...
  return (
        ...
        <img onClick={onClose} className="cu-p" src="img/btnRemove.svg" alt="Close" />
        </h2>
   ...
   ( <div className="d-flex flex-column flex cartNotEpty">
      ...
      <Info 
        ...
        description={isOrderComplite ? `Ваш заказ #${orderId} скоро будет передан курьерской доставке` : "Добавьте хотябы одну пару кроссовок"} 
        image={isOrderComplite ? "img/order-done.jpg" : "img/cart-drawer.jpg"} />
         )
        }
    </div>
     </div> 
